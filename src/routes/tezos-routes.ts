...
const router = require('express').Router()
  ...
  router.post('/kpi', (req: any, res: any)=>{
    
    UmbrellaModel.find(async function(err: any, result: any){
      try{
      if(err){
          console.log(err)
      }
        ...
        var sumOfRealizedRewards: number = 0
        var arrayOfRealizedRewards = []
        var fiatHolder: any[] = []
        result.forEach((value: any)=> {
          sumOfRealizedRewards += value.aggregateRealizedNativeReward100p
          arrayOfRealizedRewards.push(value.realizedNativeRewards)
          //make a dictionary for every new fiat 
          //check if the value matches any dictionary titles
          fiatHolder.forEach((holderItem: any)=>{
            if(holderItem){
              let dictionaryViaLiteral = {
                'Fiat': value.fiat,
                'TotalAggregateRealizedNativeReward':value.aggregateRealizedNativeReward100p,
                'IndividualSetRealizedNativeRewardArray': value.realizedNativeRewards,
              };

              fiatHolder.push(dictionaryViaLiteral)
            }else{
              if(value.fiat === holderItem[value.fiat]){
          ...
          //push to end if yes
          //and aggreagte
          //create new dictionary entry if no 
          //populate new dictoinary entry with push
          //aggregate 

        })
        // result.((value: any =>{
        //     value.aggregateRealizedNativeReward100p
        //   })
